    ...
    def process_rgb_image(self,image,image_num):
        if image is not None:
            ...
            for i in range(0, image_num):
                cv2.line(image, self.real_traj_list[i], self.real_traj_list[i+1], color=red, thickness=5)
            try:
...
def main():
    try:
        rospy.init_node("Visualizing image features")
        ...
        while not rospy.is_shutdown():
            cen=k.process_rgb_image(k.rgb_image,image_num)
            image_num += 1            
            rate.sleep()
