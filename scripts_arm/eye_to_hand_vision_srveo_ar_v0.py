...
class VisonControl():
    ...
    def get_joint_speed(self,uvm,z,desireuv,nowuv,q,X):
        ...
        # X=[-0.99863778,  0.00496206, -0.05194183 , 0.48326778,0.0498316 , -0.20448333 ,-0.97760083 , 0.53137317,-0.01547215 ,-0.97885747  ,0.20395752  ,0.41096533,0,0,0,1]
        # X=get_ur_X()#numpu array
        # print "X",X
        X=numpy.matrix(X).reshape((4,4))
        # print "X",X
        ebT=T_06
        print "T_06",T_06,type(T_06)
        #tr2jac
        jac = tr2jac(X,1)
        print "jac",jac,type(jac)
        ...
        j_speed=numpy.dot(Jacabian_joint.I,ee_speed_in_baseframe)
        return j_speed
    #
    def get_deta_joint_angular(self,detat,uvm,z,desireuv,nowuv,q,X):
        j_speed=self.get_joint_speed(uvm,z,desireuv,nowuv,q,X)
        #print j_speed
    ...
    def get_joint_angular(self,qnow,detajoint):
        ...
        return listangular
    def get_camera_to_base_1(self,q_now,ar_pose):
        T = Kinematic()
        Tbe = T.Forward(q_now)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if len(uv_get.uvlist_buf)!=0:
            ...
            camera2base=p0.get_camera_to_base(q_now, pos_dict[0])
            print "##############################################################"
            print "q_now\n", q_now
            print "joint speed\n",p0.get_joint_speed(uvlist,z,desiruv,uvlist[0],q_now,camera2base)
            print "##############################################################"
            print "deta joint angular---"
            detaangular=p0.get_deta_joint_angular(detat,uvlist, z, desiruv, uvlist[0], q_now,camera2base)
            print detaangular
            print "##############################################################"
            print "joint angular----"
