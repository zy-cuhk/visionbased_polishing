...
class VisonControl():
    ...
    def get_joint_speed(self,armaker_pose,xd,yd,zd,q):
        ...
        return j_speed
    def get_joint_speed_1(self,armaker_pose,xd,yd,zd,q,xdot,ydot,n):
        #1,get base to ee jacabian
        Jacabian_joint,T_06=self.get_jacabian_from_joint(self.urdfname,q,0)
        #2,get ee(AX=XB) to camera frame jacabian
        # X=get_ur_X()#numpu array
        ...
        print "numpy.matrix(detas).T",numpy.matrix(detas).T
        sddot=[xdot,ydot,0]
        Vc=numpy.dot(numpy.matrix(L).reshape((3,3)).I,-lamda1*numpy.matrix(detas).T+n*numpy.matrix(sddot).T)
        ...
        return j_speed
    #
    def get_deta_joint_angular(self,detat,armaker_pose,xd,yd,zd,q):
        j_speed=self.get_joint_speed(armaker_pose,xd,yd,zd,q)
        #print j_speed
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if len(uv_get.uvlist_buf)==0:
            ...
            print "##############################################################"
            print "q_now\n", q_now
            print "joint speed\n",p0.get_joint_speed(pos_dict[0][:3],xd,yd,zd,q_now)
            ...
            detaangular=p0.get_deta_joint_angular(detat,pos_dict[0][:3],xd,yd,zd,q_now)
            print detaangular
            print "##############################################################"
            print "joint angular----"
