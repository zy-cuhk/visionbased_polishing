...
class VisonControl():
    ...
    def get_cam_data(self):
        ...
        instrinct_param=yamldata['camera_matrix']['data']
        return cam
        #print yaml.load(f)
    def get_cam_data_inst(self):
        if self.sim==1:
            kx=0.008/10**(-5)
            ky=0.008/10**(-5)
            u0=512
        ...
        cam = {'kx': kx, 'ky': ky, "u0": u0, "v0": v0}
        instrinct_param=yamldata['camera_matrix']['data']
        return instrinct_param


    """ read data from yaml, here it temporary uses the list exist"""
    def get_instrinc_param(self):
        instrinc_param_data=self.get_cam_data_inst()
        data=numpy.array(instrinc_param_data)
        # data = numpy.array(
    ...
    def vis2jac1(self,uv,z):
        ...
        camf=1.0201237#0.4762192#0.6240429#m
        kx = cam['kx']
        ky = cam['ky']
        #--------------sgl-------------
        # print kx
    ...
    def get_cam_vdot(self,uvm,z,desireuv,nowuv):
        ...
        e = np.multiply( self.lambda1, e)
        vdot=numpy.dot(JJ,e.T)
        return vdot

    def get_joint_speed(self,uvm,z,desireuv,nowuv,q, ee_disturb_vel,Jacabian_joint,T_06):
        ...
        print("ee_speed_after changing--------------\n", vdot_z)
        ee_speed_compensate = ee_speed_in_base - ee_disturb_vel
        print "ee_speed-----after compensating--------", ee_speed_compensate

        j_speed=numpy.dot(Jacabian_joint.I,ee_speed_compensate)
        return j_speed
    #
    def get_deta_joint_angular(self,detat,uvm,z,desireuv,nowuv,q,ee_disturb_vel,Jacabian_joint,T_06):
        j_speed=self.get_joint_speed(uvm,z,desireuv,nowuv,q,ee_disturb_vel,Jacabian_joint,T_06)
        #print j_speed
...
def main():
    urdfname="/data/ros/ur_ws_yue/src/ur5_planning/urdf/ur3.urdf"
    filename="/data/ros/ur_ws_yue/src/ur3_controller/yaml/cam_300_final_industry.yaml"
    # urdfname="/data/ros/ur_ws/src/universal_robot/ur_description/urdf/ur5.urdf"
    desiruv=[]
    # desiruv=[[.333333333333,169]]
    lambda_u=-16.666
    lambda_v=-20
    lambda_imu = 1.05
    lambda1 = np.mat([lambda_u,lambda_v])
    detat=0.05
    z=0.84
    ace=50
    vel=0.1
...
####
    ...
    imu_sub = rospy.Subscriber("/imu", Imu, imu_reader.callback)
    ...
    while not rospy.is_shutdown():
        ...
        if len(uv_get.uvlist_buf)!=0:
            ...
            Jacabian_joint, T_06, frame_jacobian = p0.get_jacabian_from_joint(p0.urdfname, q_now, 0)
            #get joint speed in ee frame
            print "##############################################################"
            print "q_now\n", q_now
            ee_disturb_vel, imuvel = imu_reader.cal_base_vel(q_now, frame_jacobian)
            ee_disturb_vel = lambda_imu * ee_disturb_vel
            print "joint speed\n",p0.get_joint_speed(uvlist,z,desiruv,uvlist[0],q_now, imuvel, Jacabian_joint, T_06)
            print "##############################################################"
            print "deta joint angular---"
            detaangular=p0.get_deta_joint_angular(detat,uvlist, z, desiruv, uvlist[0], q_now, imuvel, Jacabian_joint, T_06)
            print detaangular
            print "##############################################################"
            print "joint angular----"
