...
"""
withforce control,just cartesian control with trejactory palnning in retangle,and use structure line with z depth
"""
class VisonControl():
    ...
    def get_joint_speed(self,armaker_pose,xd,yd,zd,q,xdot,ydot,n,f,fd,lamda2):
        ...
        print "------X",X
        inv_X_jac = jac.I
        lamda1=[-0.5,0,0,0,-0.5,0,0,0,-0.01]
        # lamda1=-0.1
        #get ee speed
        #print "tr2jac-----\n",jac
        # cam_speed = self.get_cam_vdot(uvm, z, desireuv, nowuv)
        ...
        fpie = f - self.netf_zero#f'
        fdpie = fd - self.netf_zero
        detaff_pub=rospy.Publisher("/detaf_info", Float64, queue_size=10)
        ...
        print "numpy.matrix(detas).T",numpy.matrix(detas).T
        sddot=[xdot,ydot,0]
        Vc=numpy.dot(numpy.matrix(L).reshape((3,3)).I,-numpy.matrix(lamda1).reshape((3,3))*numpy.matrix(detas).T+0*n*numpy.matrix(sddot).T+numpy.matrix(detaf).T)
            ...
            if len(sturucture_point_xn)!=0 and len(sturucture_point_yn)!=0 and len(sturucture_point_an)!=0:
                ...
                if len(xd_get.sturucture_point_xd_buf)!=0 and len(xd_get.sturucture_point_xd_buf)!=0:
                    ...
                    if len(force_list) != 0:
                        ...
                        print "joint speed\n", p0.get_joint_speed(xnynan, xd, yd, zd, q_now, xdot, 0 * ydot, count,netf_z,fd,lamda2)
                        ...
                        detaangular = p0.get_deta_joint_angular(detat, xnynan, xd, yd, zd, q_now, xdot, 0*ydot, count,netf_z,fd,lamda2)
                        print detaangular
                        print "##############################################################"
                        count += 0.5
