...
class UrLineCircle:
    ...
    def urscript_pub(self, pub, qq, vel, ace, t):
        ...
        pub.publish(ss)
    def move_ee_right(self):
        pass
    def move_ee_left(self):
        pass
    def move_ee_down(self):
        pass
    def move_ee_up(self):
        pass
def main():
    ...
    # ace=1.4
    transxyz_d=[0.05,0,0]
    qstart=[-46.705208749303715, -175.06754382908662, 80.11438968096274, -186.14081863227838, -90.21481969445425, -0.6345364125868596]
    ...
    while not rospy.is_shutdown():
        if flag==0:#left
            x_new = urc.get_draw_line_x(cn, urc.get_T_translation(ZeroT), transxyz_d)  # detax
            y_new = math.tan(urc.theta) * (x_new + TransT[0])
            print "x_new",x_new
            print "Y_new",y_new
            uree_d_x_pub.publish(x_new+TransT[0])
            uree_d_y_pub.publish(TransT[1]-math.tan(urc.theta)*x_new)
            uree_d_z_pub.publish(TransT[2])

            new_T=urc.insert_new_xy(ZeroT,x_new+TransT[0],TransT[1]+math.tan(urc.theta)*x_new,TransT[2])
        ...
        if flag==1:#right
            ...
            print "x_new", x_new
            print "Y_new", y_new
            uree_d_x_pub.publish(TransT[0]-x_new)
            uree_d_y_pub.publish(TransT[1] + math.tan(urc.theta) * x_new)
            uree_d_z_pub.publish(TransT[2])

            new_T = urc.insert_new_xy(ZeroT, TransT[0]-x_new , TransT[1] + math.tan(urc.theta) * x_new, TransT[2])
