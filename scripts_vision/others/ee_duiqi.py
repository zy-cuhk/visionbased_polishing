...
class DetectStructurePoint:
    ...
    def callback(self,data):
        try:
            ...
            print e
    def set_four_point(self,rgb_image,p1,p2,p3,p4,radius,cent):
        if rgb_image is not None:
            cv2.circle(rgb_image, (p1[0], p1[1]), int(radius),
                       (0, 0, 255), 3)
            cv2.circle(rgb_image, (p2[0], p2[1]), int(radius),
                       (0, 0, 255), 3)
            cv2.circle(rgb_image, (p3[0], p3[1]), int(radius),
                       (0, 0, 255), 3)
            cv2.circle(rgb_image, (p4[0], p4[1]), int(radius),
                       (0, 0, 255), 3)
            cv2.circle(rgb_image, (cent[0], cent[1]), int(radius),
                       (0, 0, 255), 3)
            # cv2.putText(rgb_image, "p1", (p1[0], p1[1]),
            #             cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
            # cv2.putText(rgb_image, "p2", (p2[0], p2[1]),
            #             cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
            # cv2.putText(rgb_image, "p3", (p3[0], p3[1]),
            #             cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
            # cv2.putText(rgb_image, "p4", (p4[0], p4[1]),
            #             cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
            # cv2.putText(rgb_image, "center", (cent[0], cent[1]),
            #             cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
            cv2.waitKey(8)
            cv2.namedWindow( 'Set four point in FOV', cv2.WINDOW_NORMAL )
            cv2.imshow( 'Set four point in FOV', rgb_image )
            try:
...
def main():
    try:
        ...
        cent=[328,257]
        d=50
        p1=[cent[0]-d,cent[1]+d]
        p2=[cent[0]+d,cent[1]+d]
        p3=[cent[0]-d,cent[1]-d]
        p4=[cent[0]+d,cent[1]-d]
        radius=1
        k=DetectStructurePoint()
        while not rospy.is_shutdown():
            k.set_four_point(k.rgb_image,p1,p2,p3,p4,radius,cent)
