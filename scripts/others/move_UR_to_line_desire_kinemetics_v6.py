...
class UrLineCircle:
    ...
    def caculate_vlocity_by_jocabian_pinv(self,t,Tstar,q_joint_t):
        ...
        # Jacabian_singularity=numpy.matrix(Jacabian_t).reshape(6,6)+self.p*numpy.eye(6)
        print "Jacabian_plus",Jacabian_plus
        print "tr2delta(numpy.array(Tstar).reshape(4,4),numpy.array(pose).reshape(4,4)).T",tr2delta(numpy.array(Tstar).reshape(4,4),numpy.array(pose).reshape(4,4)).T
...
def main():
    ...
    while not rospy.is_shutdown():
        if len(tool0.Ur_tool_velocity_buf) != 0:
            eVB = numpy.array(tool0.Ur_tool_velocity_buf[-1])
            WRB = numpy.array(urc.wRb).reshape(3, 3)
            print WRB
            WVB = numpy.dot(WRB, eVB.T).tolist()
            print WVB
            urc.uree_world_v_x_pub.publish(WVB[0])
            urc.uree_world_v_y_pub.publish(WVB[1])
            urc.uree_world_v_z_pub.publish(WVB[2])

        """
        go to the largest distance 
        """
        if len(ur_reader.ave_ur_pose)!=0:
            q_now_now = ur_reader.ave_ur_pose
        if flag_to_zero==0:
            cn+=1
        ...
        if flag_to_right==1:
            ...
            qq=urc.move_ee_to_right(pub,ur0_kinematics,QTemp,detax,cn)
            ...
            if len(q_now) != 0:
                QTemp = q_now
                # urc.urscript_pub(pub, QTemp, vel, ace, t)
                urc.tempq = q_now
            if cn==(urc.cont-2):
