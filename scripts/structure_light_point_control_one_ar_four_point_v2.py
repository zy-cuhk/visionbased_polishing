...
class VisonControl():
    ...
    def get_joint_speed_1(self,armaker_pose,xd,yd,zd,q):
        ...
        return j_speed
    def get_joint_speed(self,armaker_pose,xd,yd,zd,q,xdot,ydot,n):
        #1,get base to ee jacabian
        Jacabian_joint,T_06=self.get_jacabian_from_joint(self.urdfname,q,0)
        #2,get ee(AX=XB) to camera frame jacabian
        # X=get_ur_X()#numpu array
        ...
        print "numpy.matrix(detas).T",numpy.matrix(detas).T
        sddot=[xdot,ydot,0]
        Vc=numpy.dot(numpy.matrix(L).reshape((3,3)).I,-lamda1*numpy.matrix(detas).T+0*n*numpy.matrix(sddot).T)
        ...
        return j_speed
    #
    def get_deta_joint_angular(self,detat,armaker_pose,xd,yd,zd,q,xdot,ydot,n):
        j_speed=self.get_joint_speed(armaker_pose,xd,yd,zd,q,xdot,ydot,n)
        #print j_speed
        ...
        try:
            ...
            if len(pos_dict)!=0:
                ...
                if count<=200:
                    print "I'm here"
                    # yd=pos_dict[0][1]#m
                    # xd=pos_dict[0][0]#-0.01
                    # yd=0.01
                    # zd=pos_dict[0][2]#0.18#m
                    xdot = 0.01
                    ydot = 0.01
                    ...
                    print "joint speed\n", p0.get_joint_speed(pos_dict[0][:3], xd, yd, zd, q_now, xdot, 0 * ydot, count)
                    ...
                    detaangular = p0.get_deta_joint_angular(detat, pos_dict[0][:3], xd, yd, zd, q_now, xdot, 0*ydot, count)
                    print detaangular
                    print "##############################################################"
                    count += 0.5
                # elif count > 30 and count <= 55:
                #
                #     print "##############################################################"
                #     print "deta joint angular---"
                #     detaangular = p0.get_deta_joint_angular(detat, pos_dict[0][:3], xd, yd, zd, q_now, 0*xdot, ydot, count)
                #     print detaangular
                #     print "##############################################################"
                #     count += 1
                # elif count > 55 and count <= 105:
                #
                #     print "##############################################################"
                #     print "deta joint angular---"
                #     detaangular = p0.get_deta_joint_angular(detat, pos_dict[0][:3], xd, yd, zd, q_now, -1*xdot, 0*ydot, count)
                #     print detaangular
                #     print "##############################################################"
                #     count += 1
                # elif count > 105 and count <= 125:
                #
                #     print "##############################################################"
                #     print "deta joint angular---"
                #     detaangular = p0.get_deta_joint_angular(detat, pos_dict[0][:3], xd, yd, zd, q_now, 0*xdot, 1*ydot, count)
                #     print detaangular
                #     print "##############################################################"
                #     count += 1
                # elif count > 125 and count <= 175:
                #
                #     print "##############################################################"
                #     print "deta joint angular---"
                #     detaangular = p0.get_deta_joint_angular(detat, pos_dict[0][:3], xd, yd, zd, q_now, 1*xdot, 0*ydot, count)
